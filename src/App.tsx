...
import { getSalesInsight } from './services/geminiService';
import {
  CarIcon,
  ChartIcon,
  DollarIcon,
...
const SettingsPage = (props: SettingsPageProps) => {
  ...
  const availableStores = useMemo(() => {
    if (!isManager) return [];
    const mgrProps = props as ManagerSettingsProps;
    return [...new Set(mgrProps.consultants.map(c => c.store))].sort();
  }, [isManager, (props as any).consultants]);
  ...
  useEffect(() => {
    if (!isManager || !selectedStore) return;

    const mgrProps = props as ManagerSettingsProps;
    const { schedules, goals, setGoals } = mgrProps;
    ...
    const currentWorkingDays = goals[selectedStore]?.workingDays;

    if (workDaysInCurrentMonth >= 0 && currentWorkingDays !== workDaysInCurrentMonth) {
      setGoals({
        ...goals,
        [selectedStore]: {
          ...(goals[selectedStore] || { newGoal: 0, usedGoal: 0 }),
          workingDays: workDaysInCurrentMonth
        }
      });
    }
  }, [isManager, selectedStore, currentMonth, props]);
  ...
  const handleDayToggle = (day: number) => {
    ...
    if (isManager) {
      const mgrProps = props as ManagerSettingsProps;
      const { schedules, setSchedules } = mgrProps;
      const storeSchedule = schedules[selectedStore] || {};
      ...
      const consProps = props as ConsultantSettingsProps;
      const { schedule, setSchedule } = consProps;
      const isWorkDay = schedule[dateStr] !== false;
      setSchedule({ ...schedule, [dateStr]: !isWorkDay });
    }
  };

  const currentScheduleForCalendar = isManager
    ? (props as ManagerSettingsProps).schedules[selectedStore] || {}
    : (props as ConsultantSettingsProps).schedule;
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="bg-white dark:bg-slate-800/50 p-6 rounded-lg border border-gray-200 dark:border-slate-700">
        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-4">
          {isManager ? 'Store Schedule' : 'Work Schedule'}
        </h3>
        {isManager && (
          <div className="mb-4">
            <label
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 -mt-2 mb-4">
          Click days to toggle them on or off. This will automatically update
          'Working Days' for goals and pace calculations.
        </p>
            ...
            {calendarDays.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`h-16 rounded-md p-1.5 flex flex-col justify-between text-left transition-colors ${
                    isWorkDay
                      ? 'bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600'
                      : 'bg-gray-50 dark:bg-slate-900 hover:bg-gray-200 dark:hover:bg-slate-800 opacity-60'
                  }`}
                >
      ...
      {!isManager && (
        <section className="bg-white dark:bg-slate-800/50 p-6 rounded-lg border border-gray-200 dark:border-slate-700">
          <PayPlanCalculator
            salesData={(props as ConsultantSettingsProps).salesData}
            consultantName={(props as ConsultantSettingsProps).consultantName}
          />
        </section>
      )}
          ...
          {Object.entries(widgets).map(([key, value]) => (
            <div key={key} className="flex items-center">
              <input
                ...
                checked={value}
                onChange={() =>
                  setWidgets((prev) => ({ ...prev, [key]: !prev[key as keyof typeof prev] }))
                }
                className="h-4 w-4 rounded text-blue-600 focus:ring-blue-500 bg-gray-300 dark:bg-slate-600 border-gray-400 dark:border-slate-500"
              />
              <label
                htmlFor={`widget-${key}`}
              ...
              >
                {key.charAt(0).toUpperCase() +
                  key.slice(1).replace('Calc', ' Calculator')}
              </label>
            </div>
          ))}
        </div>
        <p className="text-xs text-slate-500 mt-4">
          Note: Widget toggle functionality is a visual placeholder to be implemented.
        </p>
      </section>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                name="notif-type"
                value="email"
                checked={notifications.type === 'email'}
                onChange={() =>
                  setNotifications((prev) => ({ ...prev, type: 'email' }))
                }
              />
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                name="notif-type"
                value="text"
                checked={notifications.type === 'text'}
                onChange={() =>
          ...
          <div>
            ...
            <input
              ...
              className="block w-full sm:w-1/2 rounded-md border-0 py-1.5 bg-gray-200 dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-slate-600 focus:ring-2 focus:ring-inset focus:ring-blue-500"
            />
          </div>
        </div>
        <p className="text-xs text-slate-500 mt-4">
          Note: This is a UI placeholder. The backend service required to send
          notifications has not been implemented.
        </p>
      </section>
    </div>
  );
...
const ConsultantDashboard = (props: any) => {
  const {
    ...
    handleUnwindSale,
    displayedDeliveredSales,
    consultantOtherIncomes,
    otherIncomesForCurrentView,
    isOtherIncomeFormVisible,
    setIsOtherIncomeFormVisible,
    addOtherIncome,
    updateOtherIncome,
    deleteOtherIncome,
        ...
        <div className="border border-gray-200 dark:border-slate-700 rounded-b-xl p-4 sm:p-6 bg-gray-50 dark:bg-slate-800/50">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 no-print">
            <h2 className="text-xl sm:text-2xl font-semibold text-slate-900 dark:text-white">
              ...
              <span className="text-blue-600 dark:text-blue-400">
                {currentView === 'YTD'
                  ? 'Year-to-Date'
                  : months.find((m: string) => m.startsWith(currentView))}
              </span>
            </h2>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      name="unitGoal"
                      type="number"
                      value={goals.unitGoal || ''}
                      onChange={handleGoalChange}
                      placeholder="0"
                      className="w-full rounded-md border-0 py-1 bg-gray-200 dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-slate-600 focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    <label
                      htmlFor="commissionGoal"
                      className="text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap"
                    >
                      Income Goal $
                    </label>
                ...
                <div className="md:col-span-2 bg-gray-100 dark:bg-slate-900/50 p-4 rounded-md">
                  <div className="grid grid-cols-3 text-center text-sm">
                    ...
                    <div
                      className={`py-2 border-t border-gray-200 dark:border-slate-700 font-bold text-xl ${paceColor}`}
                    >
                      ~{stats.monthlyPace.toFixed(1)}
                    </div>
                    ...
                    <div
                      className={`py-2 border-t border-gray-200 dark:border-slate-700 font-bold text-xl ${commissionPaceColor}`}
                    >
                      ~
                      {`$${stats.commissionPace.toLocaleString('en-US', {
                  ...
                  <div className="pt-3 mt-3 border-t border-gray-200 dark:border-slate-700 text-center">
                    <p className="text-sm text-slate-600 dark:text-slate-400">
                      ...
                      <strong className="text-slate-900 dark:text-white text-base mx-1">
                        {stats.unitsPerDayToGoal.toFixed(1)}
                      </strong>
                      units per day for the remaining
                      <strong className="text-slate-900 dark:text-white text-base mx-1">
                        {stats.sellingDaysLeft}
                      </strong>
                      selling days.
                    </p>
                  </div>
          ...
          <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-8">
            <StatCard
              ...
              icon={<CarIcon />}
              onClick={() => handleStatCardClick('Total Sales')}
              className="col-span-2 xl:col-span-1"
              footer={
                stats.pendingSalesCount > 0
                  ? `+${stats.pendingSalesCount} Pending`
                  : undefined
              }
              subValue={`New: ${stats.newVehicles} / Used: ${stats.usedVehicles}`}
            />
            <StatCard
              title="Total Gross"
              value={`$${stats.totalGross.toLocaleString()}`}
              icon={<DollarIcon />}
              onClick={() => handleStatCardClick('Total Gross')}
            />
            <StatCard
              title="Front PVR"
              value={`$${stats.avgFrontGross.toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
              })}`}
              icon={<DollarIcon />}
              onClick={() => handleStatCardClick('Front PVR')}
            />
            <StatCard
              title="Back PVR"
              value={`$${stats.avgBackGross.toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
              })}`}
              icon={<DollarIcon />}
              onClick={() => handleStatCardClick('Back PVR')}
            />
            <StatCard
              title="Total Commission"
              value={`$${stats.totalCommission.toLocaleString()}`}
              icon={<TagIcon />}
              onClick={() => handleStatCardClick('Total Commission')}
              valueClassName="!text-emerald-500 dark:!text-emerald-400 font-bold"
            />
            <StatCard
              title="Other Income"
              value={`$${stats.totalOtherIncome.toLocaleString()}`}
              icon={<DollarIcon />}
              onClick={() => handleStatCardClick('Other Income')}
            />
            <StatCard
              title="Avg. Commission"
              value={`$${stats.avgCommission.toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
              })}`}
              icon={<ChartIcon />}
            ...
            <StatCard
              ...
              icon={<TradeIcon />}
              onClick={() => handleStatCardClick('Total Trades')}
            />
            <StatCard
              title="Trade %"
              value={`${stats.tradePercentage.toFixed(1)}%`}
              icon={<PercentIcon />}
            />
          </section>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-2xl border border-gray-200 dark:border-slate-700">
              ...
              {salesForCurrentView.length > 0 ||
              otherIncomesForCurrentView.length > 0 ? (
                <SalesChart data={chartData} />
              ) : (
                <div className="flex items-center justify-center h-[300px] text-slate-500 dark:text-slate-400">
                  No delivered sales for this period.
                </div>
              )}
            </div>
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-2xl border border-gray-200 dark:border-slate-700 flex flex-col no-print">
              <h2 className="text-xl font-semibold mb-4 text-slate-900 dark:text-white">
              ...
              <div className="flex-grow">
                ...
                {insight && (
                  <p className="text-slate-600 dark:text-slate-300 whitespace-pre-wrap font-mono text-sm">
                    {insight}
                  </p>
                )}
                {!insight && !isGeneratingInsight && !error && (
                  <p className="text-slate-500 dark:text-slate-400">
                    Click to get AI-powered analysis for {currentView}.
                  </p>
                )}
              </div>
              <button
                onClick={handleGetInsight}
                disabled={isGeneratingInsight || salesForCurrentView.length === 0}
                className="mt-4 w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 dark:disabled:bg-slate-600 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2"
              >
                <SparklesIcon />
          ...
          <div className="flex justify-between items-center mb-4 gap-4 no-print">
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="block w-full rounded-md border-0 py-1.5 pl-10 bg-gray-200 dark:bg-slate-700 text-slate-900 dark:text-white ring-1 ring-inset ring-gray-300 dark:ring-slate-600 focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <button
              onClick={() => setIsFormVisible(!isFormVisible)}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-lg w-full sm:w-auto"
            >
              {isFormVisible ? 'Close Form' : '+ Add Sale'}
            </button>
          </div>
          ...
          {displayedPendingSales.length > 0 && (
            <section className="bg-gray-50 dark:bg-slate-800/50 p-4 sm:p-6 rounded-xl border-2 border-amber-500/30 dark:border-amber-400/50 mb-8">
              <h2 className="text-xl font-semibold text-amber-600 dark:text-amber-300 mb-4">
                Pending Deals ({displayedPendingSales.length})
              </h2>
              <SalesTable
                sales={displayedPendingSales}
                onSort={handleSort}
                sortConfig={sortConfig}
                editingSaleId={editingSaleId}
                editingSaleData={editingSaleData}
          ...
          <section className="bg-white dark:bg-slate-800 p-0 sm:p-6 rounded-xl sm:border border-gray-200 dark:border-slate-700">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4 px-4 sm:px-0 pt-4 sm:pt-0">
              Delivered Sales Log
            </h2>
            {displayedDeliveredSales.length > 0 ? (
              <SalesTable
                sales={displayedDeliveredSales}
                onSort={handleSort}
                sortConfig={sortConfig}
                editingSaleId={editingSaleId}
                editingSaleData={editingSaleData}
                ...
                onDelete={handleDeleteSale}
                role={'consultant'}
                onUnwind={handleUnwindSale}
              />
            ) : (
              <div className="text-center py-8 text-slate-500 dark:text-slate-400">
                {searchTerm
                  ? `No sales match "${searchTerm}".`
                  : 'No delivered sales for this period.'}
              </div>
            )}
          </section>
          ...
          <section className="mt-8 bg-white dark:bg-slate-800 p-0 sm:p-6 rounded-xl sm:border border-gray-200 dark:border-slate-700">
            <div className="flex justify-between items-center mb-4 px-4 sm:px-0 pt-4 sm:pt-0">
              ...
              <button
                onClick={() => setIsOtherIncomeFormVisible(!isOtherIncomeFormVisible)}
                className="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2 px-4 rounded-lg text-sm"
              >
                {isOtherIncomeFormVisible ? 'Close' : '+ Add Other Income'}
              </button>
            </div>
            {isOtherIncomeFormVisible && (
              <OtherIncomeForm onSubmit={addOtherIncome} consultantName={activeConsultantName} />
            )}
            {otherIncomesForCurrentView.length > 0 ? (
              <OtherIncomeTable
              ...
              <div className="text-center py-8 text-slate-500 dark:text-slate-400">
                No other income logged for this period.
              </div>
            )}
          </section>
...
const AppAuthed: React.FC = () => {
  ...
  const [role, setRole] = useState<'consultant' | 'manager'>(
    () => localStorageService.loadRole()
  );

  const [consultants, setConsultants] = useState<Consultant[]>(() =>
    localStorageService.loadConsultants()
  );
  const [activeConsultantName, setActiveConsultantName] = useState<string>(() =>
    localStorageService.loadActiveConsultantName()
  );
  ...
  const [goals, setGoals] = useState<Goals>(() =>
    localStorageService.loadGoals(activeConsultantName)
  );
  const [schedule, setSchedule] = useState<WorkSchedule>(() =>
    localStorageService.loadSchedule(activeConsultantName)
  );

  const [managerGoals, setManagerGoals] = useState<ManagerGoals>(() =>
    localStorageService.loadManagerGoals()
  );
  const [storeSchedules, setStoreSchedules] = useState<Record<string, WorkSchedule>>(
    () => localStorageService.loadStoreSchedules()
  );
  ...
  const managerPassword = localStorageService.loadManagerPassword();
  ...
  useEffect(() => {
    const loadData = () => {
      const loadedSales = localStorageService.loadSales();
      const processed = processSalesData(loadedSales);
      setAllSales(processed);
      setOtherIncomes(localStorageService.loadOtherIncomes());
  ...
  useEffect(() => {
    setGoals(localStorageService.loadGoals(activeConsultantName));
    setSchedule(localStorageService.loadSchedule(activeConsultantName));
  }, [activeConsultantName]);

  useEffect(() => {
    if (role === 'consultant' && consultants.length === 0) {
      setIsConsultantModalOpen(true);
    } else if (role === 'consultant' && consultants.length > 0 && !activeConsultantName) {
      setActiveConsultantName(consultants[0].name);
    }
  }, [role, consultants, activeConsultantName]);
  ...
  useEffect(() => {
    localStorageService.saveOtherIncomes(otherIncomes);
  }, [otherIncomes]);
  useEffect(() => {
    localStorageService.saveGoals(activeConsultantName, goals);
  }, [goals, activeConsultantName]);
  useEffect(() => {
    localStorageService.saveSchedule(activeConsultantName, schedule);
  }, [schedule, activeConsultantName]);
  useEffect(() => {
  ...
  useEffect(() => {
    localStorageService.saveManagerGoals(managerGoals);
  }, [managerGoals]);
  useEffect(() => {
    localStorageService.saveStoreSchedules(storeSchedules);
  }, [storeSchedules]);
  ...
  useEffect(() => {
    ...
    if (workDaysInCurrentMonth > 0 && goals.workingDays !== workDaysInCurrentMonth) {
      setGoals((prev) => ({ ...prev, workingDays: workDaysInCurrentMonth }));
    }
  }, [schedule, role, goals.workingDays]);
  ...
  const processCSVText = (text: string) => {
    try {
      ...
      const newSales: Partial<Sale>[] = parsedData.map((row) => ({
        ...
        salesperson:
          row['Salesperson'] || row['salesperson'] || activeConsultantName,
        newOrUsed:
          (row['New/Used'] || row['newOrUsed']) === 'New'
            ? VehicleType.NEW
            : VehicleType.USED,
        store:
          row['Store'] ||
          ...
          consultants.find(
            (c) =>
              c.name === (row['Salesperson'] || row['salesperson'])
          )?.store ||
          '',
        year:
          parseInt(row['Year'] || row['year']) || new Date().getFullYear(),
        make: row['Make'] || row['make'] || '',
        model: row['Model'] || row['model'] || '',
        trade:
          (row['Trade'] || row['trade'])?.toLowerCase() === 'true' ||
          (row['Trade'] || row['trade']) === 'Yes',
  ...
  const views = useMemo(
    () => [...months.map((m) => m.substring(0, 3)), 'YTD'],
    [months]
  );

  const deliveredSales = useMemo(
    () => consultantSales.filter((s) => s.status === 'delivered'),
    [consultantSales]
  );
  const pendingSales = useMemo(
    () => consultantSales.filter((s) => s.status === 'pending'),
    [consultantSales]
  );
  ...
  const stats: SalesStats = useMemo(() => {
    ...
    const totalGross = data.reduce(
      (sum, s) => sum + s.frontGross + s.backGross,
      0
    );
    const totalFrontGross = data.reduce((sum, s) => sum + s.frontGross, 0);
    const totalBackGross = data.reduce((sum, s) => sum + s.backGross, 0);
    const totalOtherIncome = otherIncomesForCurrentView.reduce(
      (sum, i) => sum + i.amount,
      0
    );
    const totalCommission =
      data.reduce((sum, s) => sum + s.commission, 0) + totalOtherIncome;
    const totalAccessories = data.reduce((sum, s) => sum + s.accessory, 0);
    const totalSpiffs = data.reduce((sum, s) => sum + s.spiffs, 0);
    const totalTradeSpiffs = data.reduce(
      (sum, s) => sum + s.tradeSpiff,
      0
    );
    const newVehicles = data
      .filter((s) => s.newOrUsed === VehicleType.NEW)
      .reduce((sum, s) => sum + s.count, 0);
    const usedVehicles = data
      .filter((s) => s.newOrUsed === VehicleType.USED)
      .reduce((sum, s) => sum + s.count, 0);
    ...
    if (currentView !== 'YTD') {
      ...
      if (monthIndex !== -1) {
        const totalWorkingDays =
          goals.workingDays || new Date(currentYear, monthIndex + 1, 0).getDate();
        ...
        unitsPerDayToGoal =
          sellingDaysLeft > 0 ? unitsToGoal / sellingDaysLeft : 0;
      }
    }
  ...
  const chartData = useMemo(() => {
    if (currentView === 'YTD') {
      ...
      const otherIncomesByMonth = consultantOtherIncomes.reduce(
        (acc, income) => {
      ...
      const monthlyData = months.map((monthName, index) => ({
        ...
        commission:
          (salesByMonth[index]?.commission || 0) +
          (otherIncomesByMonth[index] || 0)
      }));
      ...
      return monthlyData.map((d) => ({
        ...d,
        cumulativeSales: (cumulativeSales += d.sales),
        cumulativeCommission: (cumulativeCommission += d.commission)
      }));
    } else {
      ...
      const otherIncomesByDate = otherIncomesForCurrentView.reduce(
        (acc, income) => {
      ...
      const allDates = [
        ...new Set([
          ...
          ...Object.keys(otherIncomesByDate)
        ])
      ];
      ...
      const combinedDailyData = allDates.map((date) => ({
        ...
        commission:
          (salesByDate[date]?.commission || 0) +
          (otherIncomesByDate[date] || 0)
      }));

      const sortedDailyData = combinedDailyData.sort(
        (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
      );
      let cumulativeSales = 0,
        cumulativeCommission = 0;
      return sortedDailyData.map((d) => ({
        ...d,
        cumulativeSales: (cumulativeSales += d.sales),
        cumulativeCommission: (cumulativeCommission += d.commission)
      }));
  ...
  const addSale = (newSaleData: Omit<Sale, 'id' | 'cumulativeGross'>) => {
    const newSalesList = [...allSales, newSaleData];
    setAllSales(processSalesData(newSalesList));
    if (newSaleData.status === 'delivered' && role === 'consultant') {
      setCurrentView(
        months[new Date(newSaleData.date).getMonth()].substring(0, 3)
      );
    }
    setIsFormVisible(false);
  };

  const addOtherIncome = (newIncome: Omit<OtherIncome, 'id'>) => {
    setOtherIncomes([...otherIncomes, { ...newIncome, id: Date.now() }]);
    setIsOtherIncomeFormVisible(false);
  };

  const updateOtherIncome = (updatedIncome: OtherIncome) => {
    setOtherIncomes(
      otherIncomes.map((i) => (i.id === updatedIncome.id ? updatedIncome : i))
    );
    setEditingOtherIncomeId(null);
  };

  const deleteOtherIncome = (id: number) => {
  ...
  const handleDeleteSale = (idToDelete: number) => {
    if (window.confirm('Are you sure you want to delete this sale?')) {
      setAllSales(
        processSalesData(allSales.filter((s) => s.id !== idToDelete))
      );
    }
  };

  const handleMarkAsDelivered = (idToDeliver: number) => {
    const updatedSales = allSales.map((s) =>
      s.id === idToDeliver
        ? {
            ...s,
  ...
  const handleUnwindSale = (idToUnwind: number) => {
    if (
      window.confirm(
        'Are you sure you want to unwind this deal? It will be moved to Pending Deals.'
      )
    ) {
      const updatedSales = allSales.map((s) =>
        s.id === idToUnwind ? { ...s, status: 'pending' as const } : s
      );
      setAllSales(processSalesData(updatedSales));
      handleCancelEdit();
    }
  ...
  const handleGetInsight = useCallback(async () => {
    ...
    setInsight(null);
    try {
      setInsight(await getSalesInsight(stats));
    } catch (e) {
      setError(
  ...
  const handleEditChange = (
    ...
    else if (type === 'number' || name === 'count')
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `auto-sales-tracker-backup-${
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(importedData)) {
            setAllSales(processSalesData(importedData));
            setOtherIncomes([]);
          } else {
            if (Array.isArray(importedData.sales))
              setAllSales(processSalesData(importedData.sales));
            if (Array.isArray(importedData.otherIncomes))
              setOtherIncomes(importedData.otherIncomes);
            if (Array.isArray(importedData.consultants))
              setConsultants(importedData.consultants);
          }
        }
      } catch (err: any) {
  ...
  const handleConfirmImport = () => {
    setAllSales((prevSales) =>
      processSalesData([...prevSales, ...csvImportPreview])
    );
  ...
  const DetailTable = ({
    ...
    data: any[];
    columns: { Header: string; accessor: string; isCurrency?: boolean }[];
  }) => (
  ...
  const handleStatCardClick = (title: string) => {
    ...
    switch (title) {
      case 'Total Gross':
        dataToShow = salesForCurrentView.map((s) => ({
          customer: s.customerName,
          vehicle: `${s.year} ${s.make} ${s.model}`,
          front: s.frontGross,
          back: s.backGross,
          total: s.frontGross + s.backGross
        }));
        columns = [
          { Header: 'Customer', accessor: 'customer' },
          { Header: 'Vehicle', accessor: 'vehicle' },
          { Header: 'Front', accessor: 'front', isCurrency: true },
          { Header: 'Back', accessor: 'back', isCurrency: true },
      ...
      case 'Front PVR':
        ...
        columns = [
          { Header: 'Customer', accessor: 'customer' },
          { Header: 'Vehicle', accessor: 'vehicle' },
          { Header: 'Front Gross', accessor: 'front', isCurrency: true }
        ];
        break;
      case 'Back PVR':
        dataToShow = salesForCurrentView.map((s) => ({
        ...
        columns = [
          { Header: 'Customer', accessor: 'customer' },
          { Header: 'Vehicle', accessor: 'vehicle' },
          { Header: 'Back Gross', accessor: 'back', isCurrency: true }
        ];
        break;
      case 'Total Commission':
        ...
        columns = [
          { Header: 'Customer', accessor: 'customer' },
          { Header: 'Vehicle', accessor: 'vehicle' },
          { Header: 'Commission', accessor: 'commission', isCurrency: true }
        ];
        break;
      case 'Other Income':
        dataToShow = otherIncomesForCurrentView.map((i) => ({
          date: i.date,
          ...
          <div>
            ...
            {stats.pendingSalesCount > 0 && (
              <p className="mt-2 text-amber-400">
                There are {stats.pendingSalesCount} pending deals not included.
              </p>
            )}
          </div>
        );
          ...
          trades.length > 0 ? (
            <ul className="space-y-2 text-sm">
              {trades.map((s) => (
                <li
                  key={s.id}
                  className="p-2 bg-slate-200 dark:bg-slate-700/50 rounded-md"
                >
                  <strong className="text-slate-900 dark:text-white">
                    {s.customerName}
                  </strong>{' '}
    ...
    if (dataToShow.length > 0) {
      ...
      content = (
        <p>No data to display for this category in the selected period.</p>
      );
    }
  ...
  const handleSetFirstConsultant = () => {
  ...
  const handleManagerLogin = () => {
    if (!managerPassword) {
      ...
      localStorageService.saveManagerPassword(managerPasswordInput);
  ...
  const handleResetManagerPassword = () => {
    if (
      window.confirm(
        'Are you sure you want to reset the manager password? You will be prompted to create a new one.'
      )
    ) {
      localStorageService.removeManagerPassword();
      setManagerPasswordInput('');
      setManagerPasswordError('');
    }
  };

  const handleAddConsultant = (name: string, store: string) => {
    if (name && store && !consultants.some((c) => c.name === name)) {
      const newConsultants = [...consultants, { name, store }].sort((a, b) =>
        a.name.localeCompare(b.name)
      );
      setConsultants(newConsultants);
    } else {
      alert(
        consultants.some((c) => c.name === name)
          ? 'This consultant name already exists.'
          : 'Please provide a valid name and store.'
      );
    }
  };

  const handleDeleteConsultant = (nameToDelete: string) => {
    if (
      window.confirm(
        `Are you sure you want to delete consultant "${nameToDelete}"? This cannot be undone.`
      )
    ) {
      const newConsultants = consultants.filter(
        (c) => c.name !== nameToDelete
      );
      setConsultants(newConsultants);
  ...
  const handleUpdateConsultant = (oldName: string, updatedConsultant: Consultant) => {
    setConsultants((prev) =>
      prev.map((c) => (c.name === oldName ? updatedConsultant : c))
    );
    setAllSales((prev) =>
      prev.map((s) =>
        s.salesperson === oldName
          ? {
              ...s,
              salesperson: updatedConsultant.name,
              store: updatedConsultant.store
            }
          : s
      )
    );
    setOtherIncomes((prev) =>
      prev.map((i) =>
        i.salesperson === oldName
          ? { ...i, salesperson: updatedConsultant.name }
          : i
      )
    );
    if (activeConsultantName === oldName)
      setActiveConsultantName(updatedConsultant.name);
    localStorageService.migratePayPlan(oldName, updatedConsultant.name);
    localStorageService.migrateGoals(oldName, updatedConsultant.name);
    localStorageService.migrateSchedule(oldName, updatedConsultant.name);
  };
  ...
  const handleDownloadCSV = () => {
    ...
    const headers = Object.keys(displayedDeliveredSales[0]).filter(
      (key) => key !== 'id' && key !== 'cumulativeGross'
    );
    const csvContent = [
      headers.join(','),
  ...
  if (isLoadingData) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-slate-900 text-slate-800 dark:text-white">
        Loading Data...
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 no-print">
              ...
              {role === 'consultant' ? (
                <>
                  ...
                  {consultants.length > 0 ? (
                    <select
                      value={activeConsultantName}
                      onChange={(e) =>
                        setActiveConsultantName(e.target.value)
                      }
                      className="bg-gray-200 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-md py-1 px-2 text-slate-900 dark:text-white text-sm focus:ring-blue-500 focus:border-blue-500"
                    >
                      {consultants.map((c) => (
          ...
          <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`px-3 py-1 text-sm font-medium rounded-md transition-colors ${
                  role === 'consultant'
                    ...
                    : 'text-slate-600 dark:text-slate-300 hover:bg-gray-300/50 dark:hover:bg-slate-600/50'
                }`}
              >
                Consultant
              </button>
            ...
            />
            <button
              onClick={() => setIsDataActionsModalOpen(true)}
              className="bg-slate-500 hover:bg-slate-600 dark:bg-slate-600 dark:hover:bg-slate-700 text-white font-bold py-2 px-4 rounded-lg transition-colors text-sm flex items-center gap-2"
            >
              <DatabaseIcon /> Data Actions
            </button>
            <button
        ...
        {appView === 'dashboard' &&
          role === 'consultant' &&
          activeConsultantName && (
            <ConsultantDashboard {...consultantDashboardProps} />
          )}
        ...
        {appView === 'dashboard' && role === 'manager' && !isManagerAuthenticated && (
          <div className="text-center py-20">
            <p className="text-slate-500 dark:text-slate-400">
              Please authenticate to view the Manager Dashboard.
            </p>
          </div>
        )}
        ...
        {appView === 'settings' && role === 'manager' && isManagerAuthenticated && (
          <SettingsPage
            ...
            setSchedules={setStoreSchedules}
            goals={managerGoals}
            setGoals={setManagerGoals}
            consultants={consultants}
            theme={theme}
        ...
        >
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            Found {csvImportPreview.length} sales to import. This will add them to your
            existing log. Please review the preview below.
          </p>
          <DetailTable
          ...
          <div className="mt-6 flex justify-end gap-3">
            <button
              ...
              className="bg-gray-200 dark:bg-slate-600 hover:bg-gray-300 dark:hover:bg-slate-700 text-slate-800 dark:text-white font-bold py-2 px-4 rounded-lg"
            >
              Cancel
            </button>
            <button
        ...
        <Modal
          isOpen={isDataActionsModalOpen}
          onClose={() => setIsDataActionsModalOpen(false)}
          title="Data Actions"
          size="large"
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-100 dark:bg-slate-700/50 p-4 rounded-lg">
              <h3 className="font-semibold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                <UploadIcon /> Import Sales Log
              </h3>
              <button
                onClick={handleImportCSVClick}
                className="w-full text-left p-3 bg-white dark:bg-slate-600 hover:bg-gray-50 dark:hover:bg-slate-500 rounded-lg mb-2 transition-colors"
              >
                From CSV file
              </button>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                For Excel or Google Sheets, please save/export your data as a CSV file first.
              </p>
            </div>

            <div className="bg-gray-100 dark:bg-slate-700/50 p-4 rounded-lg">
              <h3 className="font-semibold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                <DatabaseIcon /> App Data Backup
              </h3>
              <button
              ...
              <button
                ...
                className="w-full text-left p-3 bg-white dark:bg-slate-600 hover:bg-gray-50 dark:hover:bg-slate-500 rounded-lg transition-colors"
              >
        ...
        >
          <div className="p-2">
            ...
            <p className="text-slate-600 dark:text-slate-300 mb-4">
              Please enter your name and store to get started.
            </p>
            <div className="space-y-4">
              <input
                type="text"
            ...
            <div className="mt-6 flex justify-end">
              <button
                onClick={handleSetFirstConsultant}
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg"
              >
                Save
              </button>
        ...
        <Modal
          ...
          title={managerPassword ? 'Manager Login' : 'Set Manager Password'}
        >
          <div className="p-2 space-y-4">
            <p className="text-slate-600 dark:text-slate-300">
              {managerPassword
                ? 'Please enter the password to access the manager dashboard.'
                : 'Create a password to protect the manager dashboard. (min 4 characters)'}
            </p>
            <input
              type="password"
            ...
            {managerPasswordError && (
              <p className="text-red-400 text-sm">{managerPasswordError}</p>
            )}
