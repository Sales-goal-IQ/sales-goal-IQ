...
const App: React.FC = () => {
    ...
    const [consultants, setConsultants] = useState<Consultant[]>(() => localStorageService.loadConsultants());
    ...
    const [goals, setGoals] = useState<Goals>(() => localStorageService.loadGoals(activeConsultantName));
    const [schedule, setSchedule] = useState<WorkSchedule>(() => localStorageService.loadSchedule(activeConsultantName));
    
    const [managerGoals, setManagerGoals] = useState<ManagerGoals>(() => localStorageService.loadManagerGoals());
    const [storeSchedules, setStoreSchedules] = useState<Record<string, WorkSchedule>>(() => localStorageService.loadStoreSchedules());
    ...
    useEffect(() => { localStorageService.saveOtherIncomes(otherIncomes) }, [otherIncomes]);
    useEffect(() => { localStorageService.saveGoals(activeConsultantName, goals); }, [goals, activeConsultantName]);
    useEffect(() => { localStorageService.saveSchedule(activeConsultantName, schedule); }, [schedule, activeConsultantName]);
    useEffect(() => { localStorageService.saveRole(role); }, [role]);
    useEffect(() => { localStorageService.saveConsultants(consultants); }, [consultants]);
    useEffect(() => { localStorageService.saveActiveConsultantName(activeConsultantName); }, [activeConsultantName]);
    useEffect(() => { localStorageService.saveManagerGoals(managerGoals); }, [managerGoals]);
    useEffect(() => { localStorageService.saveStoreSchedules(storeSchedules); }, [storeSchedules]);
    ...
    const handleMarkAsDelivered = (idToDeliver: number) => {
        const updatedSales = allSales.map(s => s.id === idToDeliver ? { ...s, status: 'delivered' as const, date: new Date().toISOString().split('T')[0] } : s);
        setAllSales(processSalesData(updatedSales));
    };
     ...
     const handleUnwindSale = (idToUnwind: number) => {
        if (window.confirm("Are you sure you want to unwind this deal? It will be moved to Pending Deals.")) {
            const updatedSales = allSales.map(s => s.id === idToUnwind ? { ...s, status: 'pending' as const } : s);
            setAllSales(processSalesData(updatedSales));
            handleCancelEdit();
        }
    };
    ...
    const handleGoalChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setGoals(prev => ({ ...prev, [name]: value === '' ? 0 : parseFloat(value) }));
    };
            ...
            try {
                ...
                if (window.confirm("This will overwrite all current data. Are you sure?")) {
                    if (Array.isArray(importedData)) {
                        ...
                        if (Array.isArray(importedData.sales)) setAllSales(processSalesData(importedData.sales));
                        if (Array.isArray(importedData.otherIncomes)) setOtherIncomes(importedData.otherIncomes);
                        if (Array.isArray(importedData.consultants)) setConsultants(importedData.consultants);
                    }
                }
            } catch (err: any) {
    ...
    const handleConfirmImport = () => {
        setAllSales(prevSales => processSalesData([...prevSales, ...csvImportPreview]));
    ...
    const handleAddConsultant = (name: string, store: string) => {
        if (name && store && !consultants.some(c => c.name === name)) {
            const newConsultants = [...consultants, { name, store }].sort((a, b) => a.name.localeCompare(b.name));
    ...
    if (isLoadingData) {
        return <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-slate-900 text-slate-800 dark:text-white">Loading Data...</div>;
    }
    ...
    const consultantDashboardProps = {
        ...
        stats, handleStatCardClick, chartData, isGeneratingInsight,
        error, insight, handleGetInsight, allSales,
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-slate-900 text-slate-800 dark:text-gray-200 p-4 sm:p-6 lg:p-8 font-sans">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <Modal isOpen={isConsultantModalOpen} onClose={() => {}}>
                    <div className="p-2">
                        <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Welcome!</h2>
...
const ConsultantDashboard = (props: any) => {
    const {
        ...
        goals, handleGoalChange, paceColor, commissionPaceColor,
        stats, handleStatCardClick, chartData, isGeneratingInsight,
        error, insight, handleGetInsight,
        isFormVisible, setIsFormVisible, searchTerm, setSearchTerm, addSale,
        displayedPendingSales, sortConfig, handleSort, editingSaleId,
        editingSaleData, handleEditClick, handleCancelEdit, handleUpdateSale,
        handleEditChange, handleDeleteSale, handleMarkAsDelivered, handleUnwindSale,
                ...
                <div className="border border-gray-200 dark:border-slate-700 rounded-b-xl p-4 sm:p-6 bg-gray-50 dark:bg-slate-800/50">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 no-print">
                        <h2 className="text-xl sm:text-2xl font-semibold text-slate-900 dark:text-white">
                            Performance Summary: <span className="text-blue-600 dark:text-blue-400">{currentView === 'YTD' ? 'Year-to-Date' : months.find(m => m.startsWith(currentView))}</span>
                        </h2>
                    </div>
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-2xl border border-gray-200 dark:border-slate-700 flex flex-col no-print">
                            <h2 className="text-xl font-semibold mb-4 text-slate-900 dark:text-white">Gemini Sales Insights</h2>
                            <div className="flex-grow">
                                {isGeneratingInsight && <div className="flex items-center justify-center h-full"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div></div>}
                                {error && <p className="text-red-500">{error}</p>}
                                {insight && <p className="text-slate-600 dark:text-slate-300 whitespace-pre-wrap font-mono text-sm">{insight}</p>}
                                {!insight && !isGeneratingInsight && !error && <p className="text-slate-500 dark:text-slate-400">Click to get AI-powered analysis for {currentView}.</p>}
                            </div>
                            <button onClick={handleGetInsight} disabled={isGeneratingInsight || salesForCurrentView.length === 0} className="mt-4 w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 dark:disabled:bg-slate-600 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2"><SparklesIcon />{isGeneratingInsight ? 'Analyzing...' : 'Generate Insight'}</button>
                        </div>
                    </section>
